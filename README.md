...
The scoped driver callbacks are the only recursive dialect seam. While a
callback is live, its block frame remains private in the shared neutral walk.
The driver itself closes that frame and resumes the parent; a failed callback
faults its driver after cleanup, rather than inventing a parent advancement.
